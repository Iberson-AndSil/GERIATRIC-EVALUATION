...
const DETALLES_CAIDA = [
  { 
    key: "neededMedicalAssistance", 
    label: "Necesitó asistencia médica" 
  },
  { 
    key: "couldNotGetUp", 
    label: "No pudo levantarse (15 min)" 
  },
  { 
    key: "fearOfFalling", 
    label: "Miedo a caer nuevamente" 
  }
] as const;
      ...
      {responses.hasFallen && (
        <div className="bg-orange-50 p-4 rounded-lg border border-orange-100 animate-fade-in">
          ...
          <div className="flex flex-col gap-2">
            {DETALLES_CAIDA.map((detalle) => (
              <Checkbox
                key={detalle.key}
                checked={responses[detalle.key]}
                onChange={(e) => onResponseChange(detalle.key, e.target.checked)}
              >
                {detalle.label}
              </Checkbox>
            ))}
